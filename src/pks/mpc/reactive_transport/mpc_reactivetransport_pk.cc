...
// Create miscaleneous lists.
  ...
  // AMANZI_ASSERT(tranport_pk_->domain_name() == chemistry_pk_->domain_name());
  // master_ = 1;  // Transport;
  // slave_ = 0;  // Chemistry;
...
void ReactiveTransport_PK_ATS::Initialize(const Teuchos::Ptr<State>& S) {
  ...
  Key tcc_key = Keys::getKey(domain_name_, "total_component_concentration");
  Key mol_den_key = Keys::getKey(domain_name_,  "molar_density_liquid");
...
void  ReactiveTransport_PK_ATS::ConvertConcentrationToAmanzi(Teuchos::RCP<AmanziChemistry::Chemistry_PK> chem_pk,
  ...
  Teuchos::RCP<const AmanziMesh::Mesh> mesh = S_->GetMesh(domain_name_);
